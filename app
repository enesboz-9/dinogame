...
def save_score(name, score):
    df = load_scores()
    new_row = pd.DataFrame([{"Isim": name, "Skor": score, "Tarih": datetime.now().strftime("%Y-%m-%d %H:%M")}])
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  body { background:#1a1a2e; display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:100vh; font-family:'Press Start 2P',monospace; overflow:hidden; }
  canvas { border:2px solid #00ff88; border-radius:8px; box-shadow:0 0 30px #00ff8844; display:block; max-width:100%; cursor:pointer; }
  #ui { color:#00ff88; font-size:10px; margin-top:10px; text-align:center; }
</style>
</head>
<body>
<canvas id="gameCanvas"></canvas>
<div id="ui">SPACE veya TIKLA ile ZIPLA | ArrowUp da calisir</div>
<script>
...
const dino = {
  ...
  draw() {
    ...
    ctx.beginPath(); ctx.moveTo(x, y+h*0.45); ctx.lineTo(x-w*0.4, y+h*0.6); ctx.lineTo(x, y+h*0.65); ctx.fill();
    ctx.fillStyle='#00ff88';
...
function spawnObstacle() {
  const types=[{w:0.035,h:0.25,color:'#ff6b6b'},{w:0.025,h:0.35,color:'#ff9f43'},{w:0.05,h:0.2,color:'#ff6b6b'}];
...
let clouds=[];
for(let i=0;i<5;i++) clouds.push({x:Math.random()*800, y:Math.random()*60+10, w:Math.random()*60+40, opacity:Math.random()*0.15+0.05, speed:Math.random()*0.5+0.3});
...
function gameLoop() {
  ...
  if(gameState==='playing') {
    dino.update();
    nextObstacle--;
    if(nextObstacle<=0){ spawnObstacle(); nextObstacle=Math.max(Math.floor(80+Math.random()*60-gameSpeed*2),35); }
    obstacles.forEach(o=>{ o.x-=gameSpeed; drawCactus(o); });
    obstacles=obstacles.filter(o=>o.x+o.w>-10);
    score++;
    if(score%100===0&&score!==lastSpeedUp){ lastSpeedUp=score; gameSpeed*=1.1; playBeep(660,0.1); setTimeout(()=>playBeep(880,0.15),100); }
...
function handleAction() {
  ...
  if(gameState==='waiting'||gameState==='gameover'){
    gameState='playing'; dino.reset(); score=0; gameSpeed=3; lastSpeedUp=0; obstacles=[]; nextObstacle=80;
  } else { dino.jump(); }
}
